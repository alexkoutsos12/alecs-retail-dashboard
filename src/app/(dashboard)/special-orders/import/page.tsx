...
import { parseSpecialOrders } from "@/lib/parsers/parseSpecialOrders";
...
export default function SpecialOrdersImportPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    document.title = "Import · Special Orders";
  }, []);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const result = parseSpecialOrders(buffer);
      const { customers, importDate, totalOutstanding } = result;

      if (customers.length === 0) {
        throw new Error(
          "No outstanding special orders found in this file. Make sure the file is a RICS Customer List — Special Orders report."
        );
      }
      ...
      // Upload customers JSON to Firebase Storage
      const storagePath = `reports/${reportId}/special-orders.json`;
      const jsonBlob = new Blob([JSON.stringify(customers)], {
      ...
      toast.success("Special orders imported successfully.");
      fetchCurrentImport();
    } catch (err) {
      setErrorMessage(
  ...
  return (
    <div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full shadow-xl">
            ...
            <p className="font-body text-sm text-brand-text/70 mb-5">
              ...
              all special orders data. This cannot be undone.
            </p>
      ...
      <h1 className="font-heading text-brand-green text-2xl font-bold mb-1">
        Import — Special Orders
      </h1>
      <p className="text-brand-text/50 font-body text-sm mb-6">
        Upload a RICS Customer List — Special Orders report (.xlsx)
      </p>
          ...
          >
            ...
            {selectedFile ? (
              ...
              <p className="font-body text-sm text-brand-text/50">
                Drag &amp; drop your RICS Customer List — Special Orders (.xlsx)
      ...
      {importState === "success" && successData && (
        <div className="bg-white border-l-[3px] border-brand-green rounded p-6 mb-6">
          ...
          <div className="mb-5">
            ...
            />
            <SummaryRow
              label="Customers with Open Orders"
              value={String(successData.totalCustomers)}
            />
            <SummaryRow
              label="Total Outstanding Items"
              value={String(successData.totalOutstanding)}
            />
          </div>
            ...
            <table className="w-full text-sm font-body min-w-[560px]">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="px-4 py-2">{currentImport.totalCustomers}</td>
                  <td className="px-4 py-2">
                    {currentImport.totalOutstanding}
                  </td>
