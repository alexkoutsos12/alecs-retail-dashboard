...
import {
  parseSalesJournalPerformance,
  PerformanceTransaction,
} from "@/lib/parsers/parseSalesJournalPerformance";
...
export default function TeamPerformanceImportPage() {
  ...
  useEffect(() => {
    document.title = "Import · Team Performance";
  }, []);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const transactions: PerformanceTransaction[] =
        await parseSalesJournalPerformance(buffer, reportId, (current, total) =>
      ...
      const doUpload = async () => {
        try {
          ...
          // Upload transactions JSON to Firebase Storage
          ...
          const uniqueSalespeople = [
            ...new Set(transactions.map((t) => t.salesperson).filter(Boolean)),
          ]
            .filter((s) => s !== "Salesperson Upstairs")
            .sort();

          const uniqueCashiers = [
            ...new Set(transactions.map((t) => t.cashier).filter(Boolean)),
          ].sort();

          const regularSales = transactions.filter(
            (t) => t.transactionType === "Regular Sale"
          ).length;
          const returns = transactions.filter((t) => t.isReturn).length;
      ...
      {importState === "success" && successData && (
        <div className="bg-white border-l-[3px] border-brand-green rounded p-6 mb-6">
          ...
          <div className="mb-5">
            ...
            />
            <SummaryRow
              label="Salespeople Found"
              value={
                successData.uniqueSalespeople.length > 0
                  ? `${successData.uniqueSalespeople.length} (${successData.uniqueSalespeople.join(", ")})`
                  : "None"
              }
            />
            <SummaryRow
              label="Cashiers Found"
              value={
                successData.uniqueCashiers.length > 0
                  ? `${successData.uniqueCashiers.length} (${successData.uniqueCashiers.join(", ")})`
                  : "None"
              }
            />
            <SummaryRow
            ...
            <table className="w-full text-sm font-body min-w-[720px]">
              <thead>
                <tr className="border-b border-brand-cream-dark text-left text-brand-text/50">
                  ...
                  <th className="px-4 py-2 font-normal">Salespeople</th>
                  <th className="px-4 py-2 font-normal">Cashiers</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-2">
                        {row.uniqueSalespeople?.length ?? 0}
                      </td>
                      <td className="px-4 py-2">
                        {row.uniqueCashiers?.length ?? 0}
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        {row.uploadedAt
